...
# DMC 7/7/25: Testing function - ERASE
first_ANC_test <- 1
late_ANC_test <- 1
...
triplex_out_func <- function(first_ANC_test=1, late_ANC_test,      # 1=test, 0=no test
  ...
  out <- list("child_outcomes" = child_outcomes, "maternal_costs" = maternal_costs_split)
...
child_db <- triplex_out_func(late_ANC_test = 1, 
                             syph_test = TRUE,
...
write.csv(child_hiv_syph, here("output", "dual_child_model.csv"))
