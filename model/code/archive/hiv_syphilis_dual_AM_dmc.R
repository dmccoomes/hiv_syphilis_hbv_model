...
# Running Kenya parameters
country <- "Kenya"
population <- "General"
source(here("code", "test_frequency.R"))
# setting up for late ANC retesting only
test_list <- test_freq_function(test_frequency = "Late ANC")
...
# Set up adjustable parameters here
PrEP <- 0
...
update_stage <- function(start_week, end_week, inc_rate, test_prob, att_prob, test_sens, stage_number) {
  for (x in start_week:end_week) {
    ...
    # DMC adding maternal mortality rates
    survival_rate <- ifelse(start_week %in% c(delivery:(delivery+early_pp_visit-1)), (1-mort_maternal_perinatal), (1-mort_adult_female))
